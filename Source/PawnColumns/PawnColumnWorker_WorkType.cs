...
namespace WorkTab
{
    public class PawnColumnWorker_WorkType : PawnColumnWorker, IAlternatingColumn, IExpandableColumn
    {
        ...
        public Vector2 LabelSize
        {
            get
            {
                if ( cachedLabelSize == Vector2.zero ) cachedLabelSize = Text.CalcSize( def.workType.labelShort );
        ...
        private Rect GetLabelRect( Rect headerRect )
        {
            ...
            var result = new Rect( x - LabelSize.x / 2f, headerRect.y, LabelSize.x, LabelSize.y );
            if ( MoveDown )
                result.y += 20f;
            return result;
        }
        ...
        private float GetValueToCompare( Pawn pawn )
        {
            ...
            if ( pawn.story        != null && pawn.WorkTypeIsDisabled( def.workType ) ) return -1f;
        ...
        private void HandleInteractionsDetailed( Rect rect, Pawn pawn )
        {
              ...
              && Mouse.IsOver( rect ) )
            {
                ...
                var oldpriority = pawn.GetPriority( def.workType, VisibleHour );
                ...
                var newPriority = pawn.GetPriority( def.workType, -1 );
        ...
        private void HandleInteractionsToggle( Rect rect, Pawn pawn )
        {
            ...
            {
                // track priority so we can play appropriate sounds
                var active = pawn.GetPriority( def.workType, VisibleHour ) > 0;
