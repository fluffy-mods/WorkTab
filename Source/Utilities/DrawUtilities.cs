...
namespace WorkTab
{
    public static class DrawUtilities
    {
        ...
        private static Color ColorOfPriority(int priority)
        {
            ...
            int staticColors = Settings.Get().priorityColors.Count;
            int rest = Settings.Get().maxPriority - staticColors + 1;

            if (staticColors < 2) //Just in case!
                return Color.Lerp(Color.green, Color.grey, ((float)(priority - 1)) / (Settings.Get().maxPriority - 1));

            if (priority < staticColors)
            {
                Color c = new Color();
                ColorUtility.TryParseHtmlString("#" + Settings.Get().priorityColors[priority - 1], out c);
                return c;
            }
            
            // lerp the last two colors
            Color color = new Color();
            ColorUtility.TryParseHtmlString("#" + Settings.Get().priorityColors[staticColors - 2], out color);
            Color lastColor = new Color();
            ColorUtility.TryParseHtmlString("#" + Settings.Get().priorityColors[staticColors - 1], out lastColor);
